...
public class problem_2485_refactoring_1 {
    public static void main(String[] args) throws IOException {
        ...
        Set<Integer> diffGCDSet;

        //가로수의 최대공약수를 정리함
        while (diff.size() != 1) {
            diffGCDSet = new HashSet<>();
            ...
            while (iter.hasNext()) {
                if (prev == 0) {
                    ...
                    diffGCDSet.add(GCD((int)iter.next(), prev));
                }
            }
        ...
        for (int i = first; i <= last; i+= diffGCD) {
            plusCount++;
        }

        //추가해서 완성된 가로수의 갯수 - 기존 가로수
        System.out.println(plusCount - items.length);
    }
